...
export default function ProjectCard(item) {
	return (
		<Link className="rounded-lg border border-slate-800 hover:border-sky-700 transition-color ease-in-out group overflow-hidden" href={`/projects/${item.slug}`}>
			<div className="flex flex-col h-full">
				<Image src={item.thumb} width={500} height={300} alt={item.title} className="p-0 w-auto" />
				<div className="p-5 grow bg-gradient-to-t from-slate-800 to-slate-800/30">
					<div className="mb-4 md:flex md:items-center md:justify-end md:space-x-2">
						{item.tags.map((tag, index) => (
							<div className="text-xs inline-flex items-center bg-slate-600 text-slate-100 rounded-full text-center p-2 mr-2 md:mr-0 h-5" key={index}>{tag}</div>
						))}
					</div>
